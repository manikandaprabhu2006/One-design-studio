...
import { ArrowLeft, MapPin, Calendar, Maximize, Building, ArrowUpRight } from "lucide-react";
      ...
      <div className="relative h-[60vh] lg:h-[70vh] overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 right-0 section-padding pb-12">
          <div className="max-w-7xl mx-auto">
            <Link href="/projects" className="inline-flex items-center gap-2 text-sm text-luxury-text hover:text-luxury-gold transition-colors mb-6">
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid lg:grid-cols-3 gap-12 lg:gap-20">
            ...
            <div className="lg:col-span-2">
              <FadeIn>
                ...
                <div className="prose prose-lg max-w-none">
                  {projectData.description.split("\n\n").map((paragraph, i) => (
                    <p key={i} className="body-lg mb-6">{paragraph.trim()}</p>
                  ))}
                </div>
              </FadeIn>
              ...
              {/* Gallery */}
              <FadeIn delay={0.2}>
                <h3 className="font-heading text-2xl text-luxury-text mb-6 mt-12">Project Gallery</h3>
                <div className="grid grid-cols-2 gap-4">
                  {projectData.images.map((img, i) => (
                    <div key={i} className={`relative aspect-[4/3] rounded-xl overflow-hidden group ${i === 0 ? "col-span-2" : ""}`}>
                      <Image
                        src={img}
            ...
            <div className="lg:col-span-1">
              <FadeIn delay={0.3}>
                <div className="glass-card rounded-2xl p-8 sticky top-32">
                  <h3 className="font-heading text-xl text-luxury-text mb-6">Project Details</h3>
                  <div className="space-y-5">
                    <div className="flex items-start gap-3">
                      <MapPin className="w-5 h-5 text-luxury-gold mt-0.5" />
                      <div>
                        <p className="text-xs text-luxury-ash uppercase tracking-wider">Location</p>
                        <p className="text-sm font-medium text-luxury-text">{projectData.location}</p>
                      </div>
                    </div>
                    <div className="flex items-start gap-3">
                      <Calendar className="w-5 h-5 text-luxury-gold mt-0.5" />
                      <div>
                        <p className="text-xs text-luxury-ash uppercase tracking-wider">Year</p>
                        <p className="text-sm font-medium text-luxury-text">{projectData.year}</p>
                      </div>
                    </div>
                    <div className="flex items-start gap-3">
                      <Maximize className="w-5 h-5 text-luxury-gold mt-0.5" />
                      <div>
                        <p className="text-xs text-luxury-ash uppercase tracking-wider">Area</p>
                        <p className="text-sm font-medium text-luxury-text">{projectData.area}</p>
                      </div>
                    </div>
                    <div className="flex items-start gap-3">
                      <Building className="w-5 h-5 text-luxury-gold mt-0.5" />
                      <div>
                        <p className="text-xs text-luxury-ash uppercase tracking-wider">Client</p>
                        <p className="text-sm font-medium text-luxury-text">{projectData.client}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <h4 className="font-heading text-lg text-luxury-text mb-4">Key Features</h4>
                    ...
                    >
                      <div className="relative aspect-[16/10] overflow-hidden">
                        ...
                        <div className="absolute bottom-0 left-0 right-0 p-6 text-white">
                          <p className="text-xs uppercase tracking-wider text-white/70 mb-1">{project.category}</p>
